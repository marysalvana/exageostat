...
static void cl_dcmg_cpu_func(void *buffers[],void *cl_arg){
    ...
    else if (kernel == 6) //differential operator
        core_dcmg_bivariate_differential_operator(A, m, n, m0, n0, l1, l2, theta, distance_metric);
...
int MORSE_MLE_dcmg_Tile_Async(MORSE_enum uplo, MORSE_desc_t *descA, location *l1,
    ...
    if(strcmp(kernel_fun, "univariate_matern_stationary")   == 0)
        ...
        kernel = 5;
    else if(strcmp(kernel_fun, "bivariate_matern_differential_operator")   == 0)
        kernel = 6;
    else
    {
